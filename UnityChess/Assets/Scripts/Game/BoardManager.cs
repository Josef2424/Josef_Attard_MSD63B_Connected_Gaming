...
public void CreateAndPlacePieceGO(Piece piece, Square position)
{
	...
	if (netObj != null)
	{
		...
		var squareNetObj = squareGO.GetComponent<Unity.Netcode.NetworkObject>();

		// If both the piece and square are valid network objects
		if (squareNetObj != null && netObj.IsSpawned && squareNetObj.IsSpawned)
		{
			// TrySetParent re‐parents the piece to the square in a netcode‐compliant way
			netObj.TrySetParent(squareNetObj, true);
		}
	}
}
