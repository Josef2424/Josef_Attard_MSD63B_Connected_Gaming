...
public class VisualPiece : MonoBehaviour
{
	...
	private void Start()
	{
		...
		GameManager.MoveExecutedEvent += OnTurnChanged;
	}
	...
	{
		// Only activate visual state if we're the owner of this piece
		...
		{
			// It's this piece's turn - highlight it
			renderer.material = highlightMaterial;
		}
		else
		{
			// Not this piece's turn - restore original
	...
	public void OnMouseDown()
	{
		...
		// Add network ownership check
		var netObj = GetComponentInParent<NetworkObject>();
		if (netObj != null && !netObj.IsOwner)
		{
			Debug.Log($"Cannot move this piece - owned by client {netObj.OwnerClientId}");
