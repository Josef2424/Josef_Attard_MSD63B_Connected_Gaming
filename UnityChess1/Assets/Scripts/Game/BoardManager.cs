...
public class BoardManager : MonoBehaviourSingleton<BoardManager>
{
	...
	{
		// Initialize the dictionary to map board squares to GameObjects.
		...
		// CHANGE: Remove the server-only check to create squares on all clients
		...
		for (int file = 1; file <= 8; file++)
		{
			for (int rank = 1; rank <= 8; rank++)
			{
				...
				// Only server should spawn the NetworkObjects
				if (NetworkManager.Singleton != null && NetworkManager.Singleton.IsServer)
				{
					var netObj = squareGO.AddComponent<Unity.Netcode.NetworkObject>();
					netObj.Spawn();
				}
	...
	public void CreateAndPlacePieceGO(Piece piece, Square position)
	{
		...
		// Get the square's GameObject and NetworkObject
		var squareGO = positionMap[position];

		// Create the piece without parenting it yet
		...
		if (netObj != null && NetworkManager.Singleton.IsServer)
		{
			...
			// Now set the transform parent (for local positioning)
